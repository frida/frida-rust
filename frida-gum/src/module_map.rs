...
impl ModuleDetails {
    ...
    pub fn with_name(name: String) -> Option<Self> {
        ...
        unsafe {
            gum_sys::gum_process_enumerate_modules(Some(save_module_details_by_name),
    ...
    pub fn name(&self) -> String {
        unsafe { CStr::from_ptr((*self.module_details).name).to_string_lossy().to_string() }
    }

    /// Get the path of this module
    pub fn path(&self) -> String {
        unsafe { CStr::from_ptr((*self.module_details).path).to_string_lossy().to_string() }
    }
...
impl ModuleMap {
    ...
    pub fn new_with_filter(filter: &mut dyn FnMut(ModuleDetails) -> bool) -> Self {
        unsafe extern "C" fn module_map_filter(details: *const gum_sys::_GumModuleDetails, callback: *mut c_void) -> i32 {
        ...
        Self::from_raw(unsafe { gum_sys::gum_module_map_new_filtered(Some(module_map_filter), &mut Box::new(filter) as *mut _ as *mut c_void, None) })
    }
    ...
    pub fn update(&mut self) {
        unsafe {
            gum_sys::gum_module_map_update(self.module_map)
        }
    }
}
...
impl Drop for ModuleMap {
    fn drop(&mut self) {
        unsafe {
            gum_sys::g_object_unref(self.module_map as *mut c_void)
        }
    }
}
