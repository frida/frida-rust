...
static void gum_rust_event_sink_finalize (GObject* obj) {
  G_OBJECT_CLASS (gum_rust_event_sink_parent_class)->finalize (obj);
}

static void gum_rust_event_sink_class_init (GumRustEventSinkClass* klass) {
  GObjectClass* object_class = G_OBJECT_CLASS (klass);
  object_class->finalize = gum_rust_event_sink_finalize;
}

static GumEventType gum_rust_event_sink_query_mask (GumEventSink* sink) {
  GumRustEventSink* self = GUM_RUST_EVENT_SINK (sink);
  return self->rust.query_mask (self->rust.user_data);
}

static void gum_rust_event_sink_start (GumEventSink* sink) {
  GumRustEventSink* self = GUM_RUST_EVENT_SINK (sink);
...
static void gum_rust_event_sink_flush (GumEventSink* sink) {
  GumRustEventSink* self = GUM_RUST_EVENT_SINK (sink);
  return self->rust.flush (self->rust.user_data);
}

static void gum_rust_event_sink_stop (GumEventSink* sink) {
  GumRustEventSink* self = GUM_RUST_EVENT_SINK (sink);
